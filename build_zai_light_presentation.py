# -*- coding: utf-8 -*-
"""
Builds the complete 20-slide NirmalNode Capstone Defense presentation in PowerPoint (.pptx)
with a premium LIGHT visual style inspired by Z.ai / GLM's design aesthetic.

Key Directives:
- Pure white / very light neutral canvas (#FFFFFF)
- Dark charcoal / near-black typography (#0F172A)
- Soft gray secondary text (#64748B, #94A3B8)
- One restrained accent color: Precision High-Tech Cyan / Cobalt (#0284C7)
- Open canvas layout: Text sits directly on the canvas without cards, boxes, or rounded containers
- High contrast, airy, generous whitespace, strong editorial typography
- Incorporates 4 papers from 2026 in the literature review
- Uses authentic academic 5-layer architecture diagram (figures_academic/five_layer_architecture_academic.png)
"""
import os
import pptx
...
# Light Z.ai / GLM Palette
WHITE = RGBColor(255, 255, 255)         # #FFFFFF Pure White Canvas
DARK_TEXT = RGBColor(15, 23, 42)        # #0F172A Deep Charcoal / Slate Black
SLATE_MUTED = RGBColor(100, 116, 139)   # #64748B Secondary Text
SLATE_LIGHT = RGBColor(148, 163, 184)   # #94A3B8 Micro Labels
ACCENT = RGBColor(2, 132, 199)          # #0284C7 Precision Cyan/Cobalt Accent
LINE_COLOR = RGBColor(226, 232, 240)    # #E2E8F0 Subtle Hairline
LINE_MUTED = RGBColor(203, 213, 225)    # #CBD5E1
EMERALD = RGBColor(16, 185, 129)        # #10B981 Clean Air / Success
AMBER = RGBColor(217, 119, 6)           # #D97706 Warning / Reactive
ROSE = RGBColor(225, 29, 72)            # #E11D48 Hazard / Critical
TABLE_ROW_ALT = RGBColor(248, 250, 252) # #F8FAFC
TABLE_ROW_2026 = RGBColor(240, 249, 255)# #F0F9FF Subtle Cyan Tint
TABLE_ROW_NN = RGBColor(224, 242, 254)  # #E0F2FE NirmalNode Highlight
...
def add_header(slide, mono_tag, headline, subheadline=None):
    ...
    add_hairline(slide, Inches(0.8), Inches(0.4), Inches(11.733), 0.015, LINE_COLOR)

    # Header text block sitting directly on canvas (NO BOX)
    ...
    p_head.font.color.rgb = DARK_TEXT
...
p_spec.font.color.rgb = ACCENT

# Right side: Hotspot image sitting with subtle hairline frame
if os.path.exists(get_path("real_world_hotspots.jpg")):
    s1.shapes.add_picture(get_path("real_world_hotspots.jpg"), Inches(8.4), Inches(1.3), width=Inches(4.133), height=Inches(4.8))
    tb_c = s1.shapes.add_textbox(Inches(8.4), Inches(6.15), Inches(4.133), Inches(0.4))
    tb_c.text_frame.margin_left = tb_c.text_frame.margin_top = 0
    p_ic = tb_c.text_frame.paragraphs[0]
    p_ic.text = "FIG 1.1 // INDUSTRIAL EXPOSURE HOTSPOTS (BANGLADESH)"
    p_ic.font.size = Pt(7.5)
    p_ic.font.bold = True
    p_ic.font.color.rgb = SLATE_LIGHT

# =========================================================================
# SLIDE 2: THE PROBLEM (Editorial Open Canvas)
# =========================================================================
...
if os.path.exists(get_path("real_world_hotspots.jpg")):
    ...
    tb_c2.text_frame.margin_left = tb_c2.text_frame.margin_top = 0
    ...
    tb_c2.text_frame.paragraphs[0].font.color.rgb = SLATE_LIGHT
...
p_takeaway.font.color.rgb = ACCENT
...
cols = [
    ("PASSIVE", ACCENT, "Traditional IoT Monitoring",
...
# =========================================================================
# SLIDE 4: RECENT RESEARCH LANDSCAPE (2024–2026 Table with 4x 2026 Papers)
# =========================================================================
...
for r, rdata in enumerate(papers_data):
    ...
    for c, val in enumerate(rdata):
        ...
        cell.fill.solid()
        if is_nn:
            cell.fill.fore_color.rgb = TABLE_ROW_NN
        elif is_2026:
            cell.fill.fore_color.rgb = TABLE_ROW_2026
        else:
            cell.fill.fore_color.rgb = TABLE_ROW_ALT if r % 2 == 0 else WHITE
        ...
        if is_nn:
            p.font.bold = True
            p.font.color.rgb = ACCENT if c in [0, 1, 3, 4, 5] else DARK_TEXT
...
tb_s4b.text_frame.paragraphs[0].text = "TAKEAWAY // All four 2026 papers focus on passive observation, macro modeling, or material tests. None close the loop to adaptive physical purification."
tb_s4b.text_frame.paragraphs[0].font.size = Pt(8.5)
tb_s4b.text_frame.paragraphs[0].font.bold = True
tb_s4b.text_frame.paragraphs[0].font.color.rgb = ACCENT
...
tb_s5b.text_frame.paragraphs[0].font.color.rgb = ACCENT
...
p_nn_flow.font.color.rgb = DARK_TEXT
...
p_nn_pills.font.color.rgb = DARK_TEXT
...
tb_s6b.text_frame.paragraphs[0].font.color.rgb = ACCENT
...
# =========================================================================
# SLIDE 8: END-TO-END SYSTEM ARCHITECTURE (Academic Diagram)
# =========================================================================
...
academic_arch_path = get_path("figures_academic/five_layer_architecture_academic.png")
if os.path.exists(academic_arch_path):
    s8.shapes.add_picture(academic_arch_path, Inches(0.8), Inches(1.8), width=Inches(7.6), height=Inches(4.6))
    tb_c8 = s8.shapes.add_textbox(Inches(0.8), Inches(6.45), Inches(7.6), Inches(0.35))
    tb_c8.text_frame.margin_left = tb_c8.text_frame.margin_top = 0
    ...
    tb_c8.text_frame.paragraphs[0].font.color.rgb = SLATE_LIGHT
...
if os.path.exists(get_path("fig01_hardware_composite.png")):
    ...
    tb_c9.text_frame.margin_left = tb_c9.text_frame.margin_top = 0
    ...
    tb_c9.text_frame.paragraphs[0].font.color.rgb = SLATE_LIGHT
...
if os.path.exists(get_path("extracted_figs/hardware_block_diagram.png")):
    ...
    tb_c10.text_frame.margin_left = tb_c10.text_frame.margin_top = 0
    ...
    tb_c10.text_frame.paragraphs[0].font.color.rgb = SLATE_LIGHT
...
p_flow.font.color.rgb = DARK_TEXT
...
if os.path.exists(get_path("software_block_diagram.png")):
    ...
    tb_c11.text_frame.margin_left = tb_c11.text_frame.margin_top = 0
    ...
    tb_c11.text_frame.paragraphs[0].font.color.rgb = SLATE_LIGHT
...
if os.path.exists(get_path("extracted_figs/ai_model_arena_pipeline.png")):
    ...
    tb_c12.text_frame.margin_left = tb_c12.text_frame.margin_top = 0
    ...
    tb_c12.text_frame.paragraphs[0].font.color.rgb = SLATE_LIGHT
...
if os.path.exists(get_path("extracted_figs/purifier_decision_flowchart.png")):
    ...
    tb_c13.text_frame.margin_left = tb_c13.text_frame.margin_top = 0
    ...
    tb_c13.text_frame.paragraphs[0].font.color.rgb = SLATE_LIGHT
...
if os.path.exists(get_path("extracted_figs/purification_pipeline.png")):
    ...
    tb_c14.text_frame.margin_left = tb_c14.text_frame.margin_top = 0
    ...
    tb_c14.text_frame.paragraphs[0].font.color.rgb = SLATE_LIGHT
...
stg_cols = [
    ...
    ("STAGE 02 // PARTICULATE", ACCENT, "HEPA-H13 Filter",
...
if os.path.exists(get_path("extracted_figs/dashboard_screenshot.jpg")):
    ...
    tb_c15a.text_frame.margin_left = tb_c15a.text_frame.margin_top = 0
    ...
    tb_c15a.text_frame.paragraphs[0].font.color.rgb = SLATE_LIGHT
...
if os.path.exists(get_path("extracted_figs/xai_dashboard_screenshot.jpg")):
    ...
    tb_c15b.text_frame.margin_left = tb_c15b.text_frame.margin_top = 0
    ...
    tb_c15b.text_frame.paragraphs[0].font.color.rgb = SLATE_LIGHT
...
# =========================================================================
# SLIDE 16: AI RESULTS (Massive Typographic Stats directly on Canvas)
# =========================================================================
...
if os.path.exists(get_path("extracted_figs/model_arena_comparison.png")):
    ...
    tb_c16a.text_frame.margin_left = tb_c16a.text_frame.margin_top = 0
    ...
    tb_c16a.text_frame.paragraphs[0].font.color.rgb = SLATE_LIGHT
...
if os.path.exists(get_path("extracted_figs/confusion_matrix.png")):
    ...
    tb_c16b.text_frame.margin_left = tb_c16b.text_frame.margin_top = 0
    ...
    tb_c16b.text_frame.paragraphs[0].font.color.rgb = SLATE_LIGHT
...
kpis_s16 = [
    ("10.62", "MAE (µg/m³)", "Champion M8 Blend (Baseline: 15.8)", ACCENT),
    ("17.43", "RMSE (µg/m³)", "Baseline was 24.3", DARK_TEXT),
    ("0.7334", "R² Score", "Strong fit on streaming data", DARK_TEXT),
...
if os.path.exists(get_path("extracted_figs/prequential_error_plot.jpg")):
    ...
    tb_c17a.text_frame.margin_left = tb_c17a.text_frame.margin_top = 0
    ...
    tb_c17a.text_frame.paragraphs[0].font.color.rgb = SLATE_LIGHT
...
if os.path.exists(get_path("extracted_figs/xai_feature_importance.png")):
    ...
    tb_c17b.text_frame.margin_left = tb_c17b.text_frame.margin_top = 0
    ...
    tb_c17b.text_frame.paragraphs[0].font.color.rgb = SLATE_LIGHT
...
tb_bom_lbl.text_frame.paragraphs[0].font.color.rgb = ACCENT
...
tb_s19b.text_frame.paragraphs[0].font.color.rgb = DARK_TEXT
...
s20_cols = [
    ...
    ("FUTURE RESEARCH HORIZONS", ACCENT,
    ...
    ("CORE CAPSTONE TAKEAWAY", DARK_TEXT,
...
for i, (col_title, col_color, body) in enumerate(s20_cols):
    ...
    p1.text = "\n" + body
    p1.font.size = Pt(8.5)
    p1.font.color.rgb = DARK_TEXT if i < 2 else ACCENT
...
print(f"Z.ai / GLM-styled LIGHT PowerPoint generated successfully: {output_pptx}")
