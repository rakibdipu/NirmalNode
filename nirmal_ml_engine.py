...
class AdaptiveAIEngine:
    ...
    @property
    def is_fitted(self):
        return self.arena.models[self.arena.champion_idx].fitted

    @property
    def sgd(self):
        return self.arena.models[0].model_a

    @property
    def pa(self):
        return self.arena.models[5].model_a
    ...
    def get_global_importance(self):
        ...
        normed = abs_w / total
        result = [{"name": n, "importance": round(float(v), 4), "weight": round(float(w_val), 4)}
                  for n, v, w_val in zip(FEATURE_NAMES, normed, w)]
