...
This chapter presents the entire engineering methodology behind NirmalNode: the overall system architecture; the hardware within the sensing and purification layers; the software running on the microcontroller; the communication and cloud architecture that links the physical node to the AI layer; the Adaptive AI pipeline, detailing the mathematical formulation of the incremental-learning models employed; and the decision logic that transforms a pollution forecast into a physical purifier action. Each subsystem is described with enough implementation detail (equations, pseudocode and architecture diagrams) to be reproducible.

\section{System Overview}
\label{sec:meth-overview}

As shown in Figure~\ref{fig:five-layer}, NirmalNode comprises five functional layers. (1) Environmental monitoring: physical sensors sample the air at the hotspot. (2) IoT data acquisition: the ESP32 reads, cleans, stamps and transmits sensor data. (3) Cloud data storage: historical and streaming data are persisted. (4) Adaptive AI prediction: an incrementally-updated model predicts pollution 1-2 hours ahead. (5) Local air purification: the physical cyclone-HEPA-activated-carbon unit is triggered according to the prediction.
...
The PMS5003 is a laser scattering particulate sensor which reports mass concentrations of PM1.0, PM2.5, and PM10 . A small fan draws air through an internal sensing chamber; particles in the air scatter a laser beam, and an internal photodiode measures the intensity and pattern of the scattered light, from which the sensor's onboard processor estimates particle count and size distribution, and in turn mass concentration. This sensor reports PM2.5 as its most safety-critical channel, since particles below 2.5~$\mu$m are small enough to penetrate deep into the alveolar region of the lungs (the physiological basis for the health outcomes reviewed in Section~\ref{sec:lr-occupational-health}).

\subsection{MQ135 -- General Air Quality / Gas Sensor}
MQ135 is a metal-oxide semiconductor (MOS) gas sensor whose resistance changes in the presence of a wide range of gasses including VOCs, smoke, NH3, NOx, alcohol vapor and benzene and can also provide an indirect, non-quantitative indication of raised CO2. It is a broad-spectrum sensor rather than a gas-specific sensor, so its output is used in NirmalNode as a general air-quality indicator feature, not a calibrated single-gas concentration channel.

\subsection{MQ136 -- Hydrogen Sulphide Sensor}
The MQ136 is a MOS sensor specifically designed for the detection of hydrogen sulfide (H$_2$S). H$_2$S is a gas of major importance in the chemical processing, tanning/leather, waste-treatment and food-processing industries, all common in the Bangladeshi industrial landscape, and is acutely toxic at high concentrations, making it a necessary dedicated sensing channel rather than relying solely on the broad-spectrum MQ135.

\subsection{MiCS-4514 -- Dual-Element Gas Sensor}
The MiCS-4514 contains two independent MOS sensing elements in one package, a \emph{RED} element sensitive to carbon monoxide, hydrogen, methane, ethanol and ammonia, and a \emph{OX} element sensitive to nitrogen dioxide. CO and NO$_2$ are both primary combustion byproducts expected at boiler-room and generator-room hotspots, and the two elements can be sampled simultaneously and independently. This sensor allows NirmalNode to monitor multiple industrially-relevant gasses from a single compact component.

\subsection{MP135 -- Supplementary Air Quality Sensor}
The MP135 is an additional general-purpose air-quality indicator that complements the MQ135 channel. This is because the individual MOS sensors are different in their sensitivity range, response time and cross-sensitivity to interfering gasses, and the robustness of the overall gas-sensing subsystem is improved with multiple, partially overlapping gas sensors. By combining the outputs of several sensors as separate input features, the downstream AI model (Section~\ref{sec:meth-ai-pipeline}) implicitly learns to weight and combine them appropriately for the specific hotspot in which the unit is deployed.

\subsection{BME280 (Optional) -- Temperature, Humidity, and Pressure}
An optional BME280 sensor gives you ambient temperature, relative humidity and atmospheric pressure readings. These environmental variables are incorporated as AI input features because the dispersion of pollutants, the suspension of particulates, and the response of gas sensors are all known to be modulated by temperature and humidity, and because diurnal pressure/temperature cycles are associated with certain patterns of industrial activity (e.g. process heat cycles).The baseline environmental compensation applied across sensor channels is defined as:
\begin{equation}
...
The bench-scale prototype is powered from a laboratory DC power supply to aid in development and testing. The design target for field deployment is a Lithium-Polymer (Li-Po) battery, selected to provide the unit with portability and independence from a fixed mains connection, important for retrofitting hotspots on existing factory floors where wiring a new mains-powered appliance may be impractical or unsafe.
...
\subsection{Initial (Offline) Training and Real-World Bangladesh Baseline}
Before field deployment, we pre-train a preliminary model $f(\cdot\,;\mathbf{w}_0)$ in batch mode on the full Bangladesh Air Quality Dataset collected by Paul~\cite{paul2026bangladesh}, which contains more than 1.04~million hourly multi-pollutant observations for 103 cities in Bangladesh (including 227,016 hourly observations for Dhaka from 2000 to 2025). Supervised regression on this real-world historical baseline learns a realistic starting parameter vector $\mathbf{w}_0$ that captures the South Asian particulate and combustion gas signatures, prior to the start of live streaming adaptation on the edge unit.
\subsection{Incremental (Online) Learning and the Model Arena}
When deployed, the forecasting module is used in the \emph{prequential (test-then-train)} protocol. NirmalNode does not rely on a single static model, but instead utilizes a competitive multi-model tournament framework, called the \textbf{Model Arena}. In the Model Arena, nine different incremental models are executed in parallel on the incoming stream. Each sample $\mathbf{x}_t$ is first scored to obtain the prior prediction of each model, and then all nine models are updated incrementally using \texttt{partial\_fit()}. After a warm-up period of $N_{\text{warmup}} = 50$ samples, the Arena automatically selects the model with the lowest cumulative prequential MAE as the active production champion ($M_{\text{champion}}$) controlling the purifier actuator.
...
When a NirmalNode unit is relocated—for example, from an industrial cluster in Gazipur (where pollution peaks may coincide with garment-production shift hours) to one in Narayanganj (where dyeing-industry effluent and emissions dominate)—the incremental estimators above continue updating from the new location's data stream without any manual retraining step. This means that the parameters of the model are gradually updated to reflect the pollution signature of the new site, due to the same online-update mechanism for routine continuous improvement, a property exactly found missing from the reviewed literature in Section~\ref{sec:lr-adaptive-ai}.
...
The previous sections have described the full system architecture (Figure~\ref{fig:five-layer}), end-to-end workflow (Figure~\ref{fig:workflow}), purification pipeline (Figure~\ref{fig:purification-pipeline}) and cloud/communication architecture (Figure~\ref{fig:cloud-arch}). This section presents the remaining architecture-level diagrams: hardware block diagram, software block diagram, AI pipeline, incremental-learning pipeline, purifier decision flowchart, and multi-hotspot deployment architecture, which together provide a complete visual specification of NirmalNode.
...
{figures/incremental_learning_pipeline.png}%
}
...
The XAI module is implemented in the \texttt{AdaptiveAIEngine} class
(\texttt{nirmal\_\allowbreak ml\_\allowbreak engine.py}) with the following methods:

\begin{itemize}[leftmargin=*, itemsep=0.4em]

    \item \texttt{explain\_\allowbreak prediction(X\_sc)}:
    Computes per-feature LFC values from Equation~\ref{eq:lfc} for the
    most recently processed sample. Returns a list of dictionaries
    sorted by $|\phi_i|$ in descending order. Each dictionary contains
    the feature name, raw value, contribution, and direction
    (``up'' / ``down'').

    \item \texttt{get\_\allowbreak global\_\allowbreak importance()}:
    Computes normalized global importance from
    Equation~\ref{eq:global-importance} using the current
    $\mathbf{w}_{\mathrm{blend}}$.

    \item \texttt{get\_\allowbreak shap\_\allowbreak explanation()}:
    Provides an optional offline SHAP explanation using
    \texttt{shap.\allowbreak LinearExplainer}. It requires the SHAP package
    (\texttt{pip install shap}) and uses the last $n$ history samples
    as the background distribution. It returns the SHAP values and
    the expected value (base rate).

\end{itemize}

Algorithm 3 summarizes the per-sample XAI computation embedded in
\texttt{process\_\allowbreak stream\_\allowbreak sample()}:
...
This chapter has presented the complete NirmalNode methodology, including a 5-layer system architecture; hardware consisting of an ESP32 controller, PMS5003 particulate sensor, MQ135/MQ136/MiCS-4514/MP135 gas sensors, optional BME280 environmental sensor, and a cyclone--HEPA--activated-carbon purification train; an EMA-filtered sensing firmware loop; a MQTT/HTTPS-based edge--cloud communication and storage architecture; an Adaptive AI pipeline based on incremental-learning estimators (SGD Regressor, Passive-Aggressive Regressor) with continuous updating and adaptation to new deployment areas without full retraining; a threshold-based Decision Engine that translates a 1--2 hour-ahead pollution forecast into a predictive rather than reactive purifier activation command; and an Explainable AI (XAI) module utilizing Linear Feature Contribution (LFC) to provide transparent, per-prediction and global feature-importance explanations. Chapter 4 describes the experimental setup, used to test this methodology and presents the corresponding results.
