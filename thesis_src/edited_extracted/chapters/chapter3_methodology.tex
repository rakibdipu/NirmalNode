...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/edge_cloud_architecture.png}
...
% Main vertical flow
\node[ilbox] (i1)
    {New Sensor Record $(\mathbf{x}_t,y_t)$ Arrives};

\node[ilbox, below=of i1] (i2)
    {Predict $\hat{y}_{t+h}$ Using Current Model $\mathbf{w}_t$};

\node[dcbox, below=of i2] (drift)
    {Drift / Relocation Detected?};

\node[ilbox, below=of drift, fill=cyan!10] (i3)
    {Score Prediction vs.\ True $y_{t+h}$};

\node[ilbox, below=of i3] (i4)
    {Update Model: $\mathbf{w}_t \rightarrow \mathbf{w}_{t+1}$};

% Area-specific re-adaptation
\node[adbox, right=2.0cm of drift] (adapt)
    {Area-Specific\\Re-Adaptation};
...
% Re-adaptation to model update
\draw[arr]
    (adapt.south) |- (i4.east);

% ------------------------------------------------
% Updated model loops back to the next sensor record
% Left-side loop, going upward
% ------------------------------------------------

\draw[arr]
    (i4.west)
    -- ++(-1.2cm,0)
    |- node[left,pos=0.72,font=\scriptsize]{Next $t$}
    (i1.west);
...
\begin{tikzpicture}[
  node distance=0.8cm,
  dfbox/.style={rectangle, draw=black, thick, rounded corners, fill=blue!8,
                minimum width=5.5cm, minimum height=0.9cm, font=\small, align=center},
  dfdec/.style={diamond, draw=black, thick, fill=red!10,
                minimum width=3.2cm, minimum height=1.4cm, font=\small, align=center, inner sep=2pt},
  arr/.style={thick,-{Stealth[length=2.2mm]}}
]
\node[dfbox] (f1) {Obtain Forecast $\hat{y}_{t+h}$};
\node[dfdec, below=0.7cm of f1] (f2) {$\hat{y}_{t+h} \geq \tau_{\text{hazard}}$?};
% OFF branch: left
\node[dfbox, below left=1.2cm and 2.0cm of f2, fill=green!12, minimum width=3.2cm] (f3) {Purifier OFF};
% ON branch: right
\node[dfbox, below right=1.2cm and 2.0cm of f2, fill=orange!15, minimum width=3.8cm] (f4) {Purifier ON\\(Pre-emptive)};
% Log node: centred below both
\node[dfbox, below=3.8cm of f2, fill=cyan!10, minimum width=5.5cm] (f5)
      {Log Decision + Update Model\\(Eq.~\ref{eq:sgd}/\ref{eq:pa})};
\draw[arr] (f1) -- (f2);
\draw[arr] (f2) -| node[above, pos=0.25, font=\scriptsize]{No}  (f3);
\draw[arr] (f2) -| node[above, pos=0.25, font=\scriptsize]{Yes} (f4);
\draw[arr] (f3) |- (f5);
\draw[arr] (f4) |- (f5);
\end{tikzpicture}%
}
...
\begin{algorithm}[htbp]
\caption{Per-sample LFC Explainability (appended to Algorithm~\ref{alg:online-learning})}
\label{alg:xai}
\begin{algorithmic}[1]
