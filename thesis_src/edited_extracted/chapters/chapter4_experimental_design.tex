...
\begin{enumerate}
    \item \textbf{ESP32 Firmware (AirGuard\_WiFi.ino):} The firmware reads all sensors every 2~seconds, applies the DEMO\_MODE compensation for non-functional MiCS-4514 channels (deriving plausible CO/NO$_2$ ADC values from the working MQ135 and MQ136), formats the complete sensor record as a JSON payload, and HTTP-POSTs it to the Python backend at \texttt{/api/ingest}. The relay on GPIO~25 (D25) is driven HIGH/LOW based on the \texttt{fan} field of the server's JSON response.
...
MiCS RED & CO                   & 10 & (simulated$^\dagger$) & DEMO\_MODE \\
MiCS NOX & NO$_2$               & 10 & (simulated$^\dagger$) & DEMO\_MODE \\
MP135   & VOC (relative)        & 20 & 7.1--11.6 & Valid \\
\bottomrule
\multicolumn{5}{@{}l@{}}{\footnotesize $^\dagger$ MiCS-4514 ADC values derived from MQ135/MQ136 in DEMO\_MODE; real hardware $R_0$ to be measured when MiCS-4514 is replaced.}
\end{tabular}
\end{table}
...
Table~\ref{tab:exp-model-comparison} presents the prequential accuracy metrics for the SGD+PA ensemble implemented in NirmalNode, evaluated over 499 labelled samples from the simulation dataset. The blended SGD+PA model (60\% SGD, 40\% PA, as per Equation~\ref{eq:blend}) achieved a final rolling MAE of 10.616~$\mu$g/m$^3$ and RMSE of 17.431~$\mu$g/m$^3$ after convergence, with an R$^2$ of 0.7334. The hazard-classification performance (purifier trigger at PM2.5 $\geq$ 35.5~$\mu$g/m$^3$) reached Precision = 0.916, Recall = 0.965, and F1-score = 0.940 over 500 samples, with only 10 false positives and 4 false negatives out of 500 test events.

\begin{table}[htbp]
\centering
\caption{Predictive accuracy of the NirmalNode Adaptive AI model (prequential evaluation, 499 labelled samples)}
\label{tab:exp-model-comparison}
\small
\setlength{\tabcolsep}{5pt}
\begin{tabular}{@{}lcccccc@{}}
\toprule
\textbf{Model} & \textbf{MAE} & \textbf{RMSE} & \textbf{R$^2$} & \textbf{Prec.} & \textbf{Recall} & \textbf{F1} \\
\midrule
SGD (standalone)            & 11.2  & 18.4  & 0.712 & 0.901 & 0.951 & 0.925 \\
PA (standalone)             & 12.8  & 20.1  & 0.681 & 0.888 & 0.970 & 0.927 \\
\textbf{SGD+PA (0.6/0.4)}  & \textbf{10.616} & \textbf{17.431} & \textbf{0.733} & \textbf{0.916} & \textbf{0.965} & \textbf{0.940} \\
\bottomrule
\end{tabular}
\end{table}

\noindent The confusion matrix for the binary hazard-classification task (SGD+PA blend, 500 samples) is:
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{figures/prequential_error_plot.jpg}
\caption{Prequential (test-then-train) rolling MAE and RMSE of the SGD+PA Adaptive AI ensemble over 500 samples. The three-phase convergence pattern (cold-start, adaptation, convergence) directly supports the thesis claim that accuracy improves continuously with site-specific data accumulation.}
...
The Linear Feature Contribution (LFC) module described in Section~\ref{sec:meth-xai} was evaluated over the same 500-sample prequential run. Figure~\ref{fig:xai-dashboard-screenshot} shows the XAI panel of the NirmalNode web dashboard displaying the top-5 feature contributions for a high-pollution sample (PM2.5~=~78~$\mu$g/m$^3$, AQI~207, purifier~ACTIVE). The bidirectional bars immediately communicate which features are pushing the forecast toward higher pollution (red, rightward) and which are counteracting the spike (green, leftward), in a format readable by a non-expert factory supervisor without any statistical training.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/xai_dashboard_screenshot.jpg}
\caption{NirmalNode Explainable AI dashboard panel (``This Prediction'' tab) showing the top-5 Linear Feature Contributions for a high-pollution sample. PM2.5~EMA (+15.47), PM10 (+14.10), PM2.5 current (+13.76), and Count $>$2.5$\mu$m (+14.65) push the forecast upward; PM2.5 $\Delta$(lag-1) ($-$27.80) partially counteracts the spike, reflecting the model's recognition that the sudden jump from a low baseline reduces the credible 1-hour projection. The ``Global Importance'' tab (right button) shows all-time learned weight magnitudes normalized to sum to 1.}
...
\midrule
ESP32 + all sensors (MQ135, MQ136, MP135, PMS5003) & Sensing / idle    & $\approx$ 1.8~W \\
ESP32 + sensors & Wi-Fi transmitting (HTTP POST) & $\approx$ 2.6~W \\
Purifier fan (12~V DC, 0.5~A rated) & OFF & 0~W \\
Purifier fan & ON (full speed) & $\approx$ 6.0~W \\
\midrule
\textbf{Total -- worst case (fan ON + Wi-Fi)} & & $\approx$ \textbf{8.6~W} \\
\textbf{Total -- standby (fan OFF, sensing only)} & & $\approx$ \textbf{1.8~W} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{MiCS-4514 DEMO\_MODE substitution.} The CO and NO$_2$ ADC channels in the submitted prototype are derived from MQ135/MQ136 readings via the firmware's \texttt{DEMO\_MODE} compensation, rather than from the physical MiCS-4514 sensor. This does not affect the PM2.5-based AQI calculation, purifier trigger, or the primary evaluation metrics, all of which are grounded in the PMS5003 output; it does mean that the CO- and NO$_2$-specific ppm estimates reported in the dashboard are approximate and should not be used for gas-specific regulatory compliance.
    \item \textbf{MOS gas sensor precision.} The MQ-series sensors used are low-cost, broad-spectrum devices with known cross-sensitivity and drift characteristics; their absolute concentration estimates are less precise than reference-grade electrochemical analyzers, even after the clean-air $R_0$ calibration.
