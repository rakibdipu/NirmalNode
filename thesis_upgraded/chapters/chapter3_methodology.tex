...
Prior to field deployment, an initial model $f(\cdot\,;\mathbf{w}_0)$ is pre-trained in batch mode on the comprehensive Bangladesh Air Quality Dataset compiled by Paul~\cite{paul2026bangladesh}, which contains over 1.04~million hourly multi-pollutant observations across 103 cities in Bangladesh (including 227,016 hourly records for Dhaka spanning 2000 to 2025). Supervised regression on this real-world historical baseline establishes a realistic starting parameter vector $\mathbf{w}_0$ reflective of South Asian particulate and combustion gas signatures before the edge unit begins live streaming adaptation.
