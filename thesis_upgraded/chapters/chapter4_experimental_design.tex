...
\begin{enumerate}
    ...
    \item \textbf{Physical Hardware Stream:} Live serial readings acquired from the physical ESP32 prototype during laboratory bench chamber tests with incense-smoke aerosol injection to validate physical 12V relay actuation and multi-stage filtration.
\end{enumerate}
...
\toprule
\textbf{ID} & \textbf{Model Architecture / Paradigm} & \textbf{MAE ($\mu$g/m$^3$)} & \textbf{RMSE ($\mu$g/m$^3$)} & \textbf{Status / Rank} \\
\midrule
\textbf{M6} & \textbf{Passive-Aggressive ($C=1.0$)} & \textbf{8.552} & \textbf{13.779} & \textbf{Elected Champion} \\
M8 & SGD+PA Blend (0.6/0.4 Ensemble) & 9.100 & 13.886 & Competitive Runner-up \\
M1 & SGD Regressor (Squared Error, $L_2$) & 9.905 & 14.529 & Stable Linear \\
M3 & SGD Regressor (ElasticNet, $\rho=0.5$) & 9.905 & 14.529 & Sparse Linear \\
M2 & SGD Regressor (Squared Error, $L_1$) & 9.905 & 14.529 & Sparse Linear \\
M9 & EMA Baseline (Naive Exponential Filter) & 15.027 & 20.626 & Non-ML Baseline \\
M7 & Passive-Aggressive ($C=0.1$) & 31.580 & 38.791 & Conservative \\
M4 & SGD Regressor (Huber Loss, $\delta=1.35$) & 53.627 & 55.338 & Robust Loss \\
M5 & SGD Regressor (Epsilon-Insensitive SVR) & 55.066 & 56.949 & Support Vector \\
\bottomrule
\end{tabular}
\end{table}
